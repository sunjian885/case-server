...
public class RoomFactory {
    ...
    public static RoomEntity getRoom(String roomId, Long caseId, TestCaseMapper caseMapper, CaseBackupMapper caseBackupMapper) {
//        synchronized (roomId) { // todo：此处暂不确定是否有效，需要进行验证

            if (roomEntityMap.containsKey(roomId)) {
                return roomEntityMap.get(roomId);
            } else {
                RoomEntity roomEntity = new RoomEntity(roomId, caseId, caseMapper,caseBackupMapper);
