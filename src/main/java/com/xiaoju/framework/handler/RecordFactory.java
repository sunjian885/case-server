...
public class RecordFactory {
    ...
    public static RecordEntity getRoom(String roomId, Long caseId, TestCaseMapper caseMapper, CaseBackupMapper caseBackupMapper, Long recordId, ExecRecordMapper recordMapper) {

        if (recordEntityMap.containsKey(roomId)) {
            return recordEntityMap.get(roomId);
        } else {
            RecordEntity roomEntity = new RecordEntity(roomId, caseId, caseMapper,caseBackupMapper, recordId, recordMapper);
