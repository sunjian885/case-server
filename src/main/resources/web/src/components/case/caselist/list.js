    ...
    const columns = [
      ...
      },
      {
        ...
        render: requirements => {
          //给多个标签设置不同的颜色
          if(requirements){
            return JSON.parse(requirements).map(item=>{
              let color = this.getRandomColor()
              return <Tag key={item.requireId} color={color}> 
                <a target="_blank" href={item.requireUrl}>{item.requireId}-{item.requireName}</a>
              </Tag>
            })
          }else{
            return (<div>没有填写关联需求</div>)
          }
        },
      },
      {
  ...
  renderExpand = item => {
    const columns = [
      ...
      },
      {
        ...
        render: text => (
          <Tooltip title={text}>
            {text.startsWith('[')?JSON.parse(text).map(name => 
