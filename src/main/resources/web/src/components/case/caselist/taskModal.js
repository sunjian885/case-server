...
class TaskModal extends React.Component {
  ...
  saveTaskData = values => {
    // console.log('saveTaskData values ===',values)
    // console.log('saveTaskData this.state.ownerSelect ===',this.state.ownerSelect)
    let params = values;
    ...
    params.owner =  this.state.ownerSelect ? JSON.stringify(this.state.ownerSelect) : '';
  ...
  getAllUsers = () => {
    request(`/user/getAllUser`, {
      ...
      if (res && res.code === 200) {
        this.setState({ loading: false, allUserInfos: res.data})
      } else {
        message.error(res.msg)
      }
    })
  ...
  selectChange =(value)=>{
    ...
    this.setState({ownerSelect: value})
  }
  ...
  render() {
    ...
    const { choiseDate, radioValue, selectValue, resource, ownerSelect} = this.state;
    const radioStyle = {
      display: 'block',
      height: '80px',
      marginTop: '10px',
      ...
      >
        ...
        <Form.Item {...formItemLayout} label="负责人">
            {getFieldDecorator('owner', {
                    initialValue: this.props.titleModeTask == '编辑测试任务'
                    ? JSON.parse(this.props.record.owner)
                    : [],
                })(
              <Select
                mode="multiple"
              ...
              >
                {this.state.allUserInfos.map(info =>{
                  return <Option value={info.realName} label={info.realName} key={info.id}>
                  {info.realName}
                </Option>
                })}
